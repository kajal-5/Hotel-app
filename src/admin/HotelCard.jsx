...
import EditHotel from "./EditHotelButton"; // ✅ correct file name
...
const HotelCard = ({ hotel, requests = [] }) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    const updates = {
      ...
      availablePeople: updatedAvailable,
    ...
    <>
      <Card className="h-100 shadow-sm">
        ...
        <Card.Body>
          ...
          <Card.Text>{hotel?.description || "No description provided"}</Card.Text>
          ...
          <div>
            ...
            {requests.length === 0 ? (
              ...
              requests.map((r) => (
                ...
                >
                  <div>
                    <div><strong>{r.userEmail?.split("@")[0]}</strong></div>
      ...
      {/* ✅ Correct modal rendering (no <button> wrapper) */}
      <EditHotel
        show={showEdit}
...
//   // ✅ Prevent crash if hotel data not ready
//   if (!hotel) {
//     return (
//       <div className="text-center text-muted py-3">
//         Loading hotel data...
...
//   const handleDelete = async () => {
//     if (!window.confirm(`Delete hotel "${hotel.name}"?`)) return;
//     await dispatch(deleteHotel(hotel.id)); // deletes from Firebase
//     alert("Hotel deleted successfully");
//   };
...
//     e.preventDefault();
//      const newTotal = Number(form.people);
//   const oldAvailable = editing.availablePeople ?? editing.totalPeople ?? 0;

//   // ✅ New logic: add new total to existing available count
//   const updatedAvailable = Math.max(0, oldAvailable + newTotal);
//     const updates = {
//       name: form.name,
//       img: form.img,
//       description: form.description,
//       totalPeople:updatedAvailable,
//       price: Number(form.price),
//     };
//     await dispatch(updateHotel({ id: hotel.id, updates }));
//     setShowEdit(false);
